    ...
    /*手机端用户登陆后台*/
        public function login(){
            ...
            if($check){
                ...
                $infoQuery1=$infoTable1->where("userAccount='$userAccount'")->getField("childID,childIcon,childName,childBirthdate,childSex,birthCity,fatherHeight,motherHeight,fullMonth,bearingAge");//查询出该手机用户关联的儿童的信息
                $dataTable1=M("measureData");
                $dataQuery1=$dataTable1->where("userAccount='$userAccount'")->getField("importTime,childID,childHeight,childWeight,childHeadc,childBMI");//查询出该手机用户关联的儿童账号的身高体重头围BMI信息
                $infoTable2=M("pregBaseInfo");
                $infoQuery2=$infoTable2->where("userAccount='$userAccount'")->getField("name,pregnantTime,expectedChildbirth,fatherHeight,motherHeight,sex,birthdate,birthHeight,birthWeight,birthHeadc,normalWeight,icon");//查询出该手机用户关联的孕妇的信息
                $dataTable2=M("pregWeight");
                $dataQuery2=$dataTable2->where("userAccount='$userAccount'")->getField("time,weight");//查询出该手机用户关联的孕妇的体重测量信息
                $dataTable3=M("bmodeData");
                $dataQuery3=$dataTable3->where("userAccount='$userAccount'")->getField("time,crl,hc,bpd,ac,fl,ofd,wg");//查询出该手机用户关联的孕妇的其他指标的测量信息
                $back=array("flag"=>1,"token"=>$token,"childInfo"=>$infoQuery1,"childData"=>$dataQuery1,"pregInfo"=>$infoQuery2,"pregWeightData"=>$dataQuery2,"pregBmodeData"=>$dataQuery3);//返回1表示登录成功,token:后续访问令牌
            }else{
...
//            $time=date("Y-m-d",time());
    ...
    public function BmodeDataAdd(){
        $token=I("post.token");//获取身份令牌
        $userAccount=I("post.userAccount");//参数判断
        $time=I("post.time");
        $crl=I("post.crl");
        $hc=I("post.hc");
        $bpd=I("post.bpd");
        ...
        if($token==$_SESSION["currentLogin"]){
           ...
           if($check){
             $back=array("flag"=>2);//返回2表示该日的数据已经存在，不能重复添加
           }else{
             $condition["userAccount"]=$userAccount;//用户账号
             ...
             $condition["ofd"]=$ofd;
             $condition["wg"]=$wg;
             $result=$table->add($condition);//将孕妇测试的B超数据添加到数据库
              if($result){
                        $back=array("flag"=>1);//返回1表示添加成功
                    }else{
                        $back=array("flag"=>0);//返回0表示添加失败
       ...
       if($token==$_SESSION["currentLogin"]){
           ...
           if($check){
             $back=array("flag"=>2);//返回2表示该日的数据已经存在，不能重复添加
              ...
              $result=$table->add($condition);//将孕妇测试的体重数据添加到数据库
              if($result){
                        $back=array("flag"=>1);//返回1表示添加成功
                    }else{
                        $back=array("flag"=>0);//返回0表示添加失败
...
//        $time=date("Y-m-d",time());
    ...
    public function BmodeDataEdit(){
        ...
        if($token==$_SESSION["currentLogin"]){
           if (isset($userAccount )&& isset($time) && isset($crl)&& isset($hc)&& isset($bpd)&& isset($ac)&& isset($fl)&& isset($ofd)&& isset($wg)) {
             ...
             $condition["ofd"]=$ofd;
             $condition["wg"]=$wg;
             $result = $table->where("userAccount='$userAccount'and time='$time'")->save($condition);//将孕妇修改的B超数据更新数据库
                if ($result!==false) {//更新成功返回发生变化的行数，更新失败会返回false，所以如果数据未发生变化会返回0，因此这里用false判断
                    $back = array("flag" => 1);//返回1表示修改成功
                } else {
                    $back = array("flag" => 0);//返回0表示修改失败
    ...
    public function BmodeDataDelete(){
        ...
        if($token==$_SESSION["currentLogin"]){
          if ($userAccount && $time) {
              $result = $table->where("userAccount='$userAccount'and time='$time'")->delete();//将需要删除的孕妇B超测量数据进行删除
                    if ($result) {
                        $back = array("flag" => 1);//返回1表示删除成功
                    } else {
                        $back = array("flag" => 0);//返回0表示删除失败
    ...
    public function pregWeightDelete(){
        ...
        if($token==$_SESSION["currentLogin"]){
          if ($userAccount && $time) {
              $result = $table->where("userAccount='$userAccount'and time='$time'")->delete();//将需要删除的孕妇体重测量数据进行删除
                    if ($result) {
                        $back = array("flag" => 1);//返回1表示删除成功
                    } else {
                        $back = array("flag" => 0);//返回0表示删除失败
...
//          $Icon=I("post.icon");
           ...
           if($token==$_SESSION["currentLogin"]) {
              $check=$table->where("userAccount='$userAccount'")->find();
               if ($check) {
                    $back = array("flag" => 2);//返回2表示新增的孕妇帐号已存在
                } else {
                      //建立孕妇的基本信息
                      $condition["name"]=$name;
...
//                    $condition["icon"]=$icon;
                      ...
                      if($result){
                           $back=array("flag"=>1);//返回1表示新增孕妇帐号成功
                        }else{
                           $back=array("flag"=>0);//返回0表示新增失败
                        }
                    }
     ...
     // 手机端孕妇的baby出生数据添加
       public function addBirthData(){
            ...
            if($token==$_SESSION["currentLogin"]) {
                 ...
                 $result = $table->where("userAccount='$userAccount'")->save($condition);//添加数据
                if ($result!==false) {//更新成功返回发生变化的行数，更新失败会返回false，所以如果数据未发生变化会返回0，因此这里用false判断
                    $back = array("flag" => 1);//返回1表示修改成功
                } else {
                    $back = array("flag" => 0);//返回0表示修改失败
